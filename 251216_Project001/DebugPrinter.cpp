...
void DebugPrinter::printErrorCode(int code) // 1: HP/MP error, 2: Attack/Defence error, 3: Attributes error
{
	switch (code)
	{
	case 1:
		...
		break;
	case 2:
		cout << "공격력과 방어력은 0을 초과하는 값이 입력되어야 합니다. 다시 입력해 주세요." << endl;
		break;
	case 3:
		cout << "힘, 민첩, 지능은 0을 초과하고 총 합이 10을 넘어가지 않아야 합니다. 다시 입력해 주세요." << endl;
		break;
	default:
...
{
	cout << "* 포션이 지급되었습니다. (HP, MP 포션 각 " << to_string(potionAmount) << "개)" << endl;
}
...
void DebugPrinter::printHPMPUp(bool isSuccess, bool isHP, int potionTarget, Status stats)
{
	if (!isSuccess)
	{
		cout << "* 포션이 부족합니다." << endl;
	...
	{
		cout << "* " << (isHP ? "HP" : "MP") << "가 20 증가되었습니다. 포션이 1개 차감됩니다." << endl;
		cout << "* 현재 " << (isHP ? "HP" : "MP") << ": " << (isHP ? stats.displayHP() : stats.displayMP()) << endl;
		cout << "* 남은 포션 수: " << to_string(potionTarget) << endl;
	}
}
void DebugPrinter::printAttackDefenceDouble(bool isAttack, Status stats)
{
	cout << "* " << (isAttack ? "공격력" : "방어력") << "이 2배로 증가되었습니다." << endl;
	cout << "* 현재 " << (isAttack ? "공격력" : "방어력") << ": " << (isAttack ? stats.displayAttack() : stats.displayDefence()) << endl;
}
void DebugPrinter::printAttributeIncrease(bool isSuccess, int attributeIndex, int attributePoint, Status stats)
{
	if (!isSuccess)
	{
...
void DebugPrinter::printOverallState(Status stats, int potionHP, int potionMP, int attributePoint)
{
	string printStr = "* ";
